...
// Singleton https://refactoring.guru/design-patterns/singleton/typescript/example
 class HoneycombDataDogForwarder {
  ...
  static forwardTrace(spans,rootSpan) {
    try {
      ...
      if (rootSpan._parentId == null) {
        ...
        rootSpan.getBaggageItem('rootSampleRate') != null ? parseInt(rootSpan.getBaggageItem('rootSampleRate'), 10) : undefined;
        const childSpanSampleRate =
        rootSpan.getBaggageItem('childSpanSampleRate') != null ? parseInt(rootSpan.getBaggageItem('childSpanSampleRate'), 10) : undefined;
        const slowTraceLimitMs =
        rootSpan.getBaggageItem('slowTraceLimitMs') != null ? parseInt(rootSpan.getBaggageItem('slowTraceLimitMs'), 10) : undefined;
  ...
  static sendTraceToHoneycomb({
    ...
    if (isSlow) {
      console.info({ traceId, duration_ms: rootSpan.duration_ms }, 'slow trace');
      // Sample all very slow traces since they are more likely to be interesting
      childSpanSampleRate = 1;
      rootSampleRate = 1;
    }

    const { shouldSample: shouldSampleTrace } = HoneycombDataDogForwarder.deterministicSampler(rootSampleRate, traceId);
    if (shouldSampleTrace) {
      ...
      const { shouldSample: shouldSampleChildSpans } = HoneycombDataDogForwarder.deterministicSampler(childSpanSampleRate, traceId);
